...
export default function FinishedProductsPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/v1/inventory/finished');
      if (res.ok) {
  ...
  const handleAdjustment = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`/api/v1/inventory/finished/${selectedProduct.id}/adjust`, {
        method: 'POST',
  ...
  return (
    <div>
      <PageHeader title={t('finishedProducts')} />
      <DataTable columns={columns} data={products} loading={loading} onExport={handleExport} />
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {selectedProduct?.nameAr} - {t('currentStock')}:{' '}
              {selectedProduct?.currentStock} {tCommon('kg')}
            </p>
